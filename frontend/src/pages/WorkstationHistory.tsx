...
import { ArrowLeft, History, User, MonitorPlay, FileText, Loader2 } from "lucide-react";
import {
    Table,
    TableBody,
    TableCell,
...
interface WorkstationDetails {
    ...
    processor: string;
    generation: string;
    graphicscard_brand: string | null;
    graphicscard_model: string | null;
    isAvailable: number | boolean;
    pyramidsID: string | null;
...
export default function WorkstationHistory() {
    ...
    const [workstation, setWorkstation] = useState<WorkstationDetails | null>(null);
    ...
    const fetchHistory = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/inventory/history/workstation/${id}`);
            if (response.data.success) {
                setWorkstation(response.data.data.workstation);
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            const response = await api.post(
                `/inventory/history/workstation/${id}/pdf`,
                {
                    ...
                    pyramidID: workstation.pyramidsID,
                },
                },
                { responseType: "blob" }
            );
            ...
            link.href = url;
            link.setAttribute("download", `Workstation_History_${new Date().toISOString().split("T")[0]}.pdf`);
    ...
    if (!workstation) {
        return (
            <div className="container mx-auto py-10 text-center">
                <h1 className="text-2xl font-bold text-red-500">Workstation Not Found</h1>
    ...
    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold tracking-tight">Workstation History</h1>
            </div>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* <div>
                            ...
                            <p className="text-lg font-semibold">{workstation.id}</p>
                        </div> */}
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Name</p>
                            <p className="text-lg font-semibold">{workstation.Name || "-"}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Processor</p>
                            <p className="text-lg font-semibold">{workstation.processor}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Generation</p>
                            <p className="text-lg font-semibold">{workstation.generation || "-"}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Graphics Card</p>
                            <p className="text-lg font-semibold">
                                {workstation.graphicscard_brand ? `${workstation.graphicscard_brand} ${workstation.graphicscard_model || ""}` : "N/A"}
                            </p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Pyramid ID</p>
                            <p className="text-lg font-semibold">{workstation.pyramidsID || "-"}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Inventory ID</p>
                            <p className="text-lg font-semibold">{workstation.inventoryID || "-"}</p>
                        </div>

                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Current Status</p>
                            <div className="mt-1">
                                {(workstation.isAvailable === 1 || workstation.isAvailable === true) ? (
