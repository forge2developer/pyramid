...
export default function SSDHistory() {
    const { id } = useParams();
    const navigate = useNavigate();
    const [loading, setLoading] = useState(true);
    const [ssd, setSSD] = useState<SSDDetails | null>(null);
    ...
    const fetchHistory = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/inventory/history/ssd/${id}`);
            if (response.data.success) {
                setSSD(response.data.data.ssd);
    ...
    const getAssignmentInfo = (item: SSDDetails) => {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            const response = await api.post(
                `/inventory/history/ssd/${id}/pdf`,
                {
                    ...
                    details: {
                        ...
                        size: ssd.ssdSize,
                        speed: ssd.speed,
                        serialNumber: ssd.SerialNumber,
                        serviceTag: ssd.serviceTag,
                    },
                },
                { responseType: "blob" }
            );
            ...
            link.href = url;
            link.setAttribute("download", `SSD_History_${new Date().toISOString().split("T")[0]}.pdf`);
    ...
    if (!ssd) {
        return (
            <div className="container mx-auto py-10 text-center">
                <h1 className="text-2xl font-bold text-red-500">SSD Not Found</h1>
    ...
    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold tracking-tight">SSD Details & History</h1>
            </div>
            ...
            <Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* <div>
                            <p className="text-sm font-medium text-muted-foreground">SSD ID</p>
                            <p className="text-lg font-semibold">{ssd.ssdID}</p>
                        </div> */}
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Brand</p>
                            <p className="text-lg font-semibold">{ssd.brand}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Size / Speed</p>
                            <p className="text-lg font-semibold">{ssd.ssdSize} - {ssd.speed}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Serial Number</p>
                            <p className="text-lg font-semibold">{ssd.SerialNumber}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Service Tag</p>
                            <p className="text-lg font-semibold">{ssd.serviceTag || "N/A"}</p>
                        </div>
                        ...
                        <div className="col-span-1 md:col-span-2">
                            ...
                            <div className="mt-1 flex items-center gap-2">
                                ...
                                <span className="font-semibold text-primary">{getAssignmentInfo(ssd)}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-1">
                                {(ssd.isAvailable === 1 || ssd.isAvailable === true) ? (
