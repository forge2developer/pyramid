...
import { ArrowLeft, History, User, Laptop, FileText, Loader2 } from "lucide-react";
import {
    Table,
    TableBody,
    TableCell,
...
interface MobileWorkstationDetails {
    id: number;
    processor: string;
    generation: string;
    display: string;
    ...
    graphicscard_brand: string;
    graphicscard_model: string;
    model: string;
...
export default function MobileWorkstationHistory() {
    const { id } = useParams();
    const navigate = useNavigate();
    const [loading, setLoading] = useState(true);
    const [mws, setMws] = useState<MobileWorkstationDetails | null>(null);
    ...
    const fetchHistory = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/inventory/history/mobileWorkstation/${id}`);
            if (response.data.success) {
                setMws(response.data.data.mobileWorkstation);
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            const response = await api.post(
                `/inventory/history/mobileWorkstation/${id}/pdf`,
                {
                    ...
                    details: {
                        model: mws.model,
                        processor: `${mws.processor_brand} ${mws.processor_model}`,
                        generation: mws.generation,
                        serviceTag: mws.service_id,
                        pyramidID: mws.phyramidID,
                    },
                },
                { responseType: "blob" }
            );
            ...
            link.href = url;
            link.setAttribute("download", `MobileWorkstation_History_${new Date().toISOString().split("T")[0]}.pdf`);
    ...
    if (!mws) {
        return (
            <div className="container mx-auto py-10 text-center">
                <h1 className="text-2xl font-bold text-red-500">Mobile Workstation Not Found</h1>
    ...
    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold tracking-tight">Mobile Workstation History</h1>
            </div>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* <div>
                            ...
                            <p className="text-lg font-semibold">{mws.id}</p>
                        </div> */}
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Model</p>
                            <p className="text-lg font-semibold">{mws.model || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Processor</p>
                            <p className="text-lg font-semibold">{`${mws.processor_brand} ${mws.processor_model}`}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Generation</p>
                            <p className="text-lg font-semibold">{mws.generation}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Pyramids ID</p>
                            <p className="text-lg font-semibold">{mws.phyramidID || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Service Tag</p>
                            <p className="text-lg font-semibold">{mws.service_id || "N/A"}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-1 flex items-center gap-2">
                                <span className="font-semibold text-primary">{mws.company_name || "Unassigned"}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-1">
                                {(mws.isAvailable === 1 || mws.isAvailable === true) ? (
