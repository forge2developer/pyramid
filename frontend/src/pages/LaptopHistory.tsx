...
import { ArrowLeft, Laptop, History, Building2, User, FileText, Loader2 } from "lucide-react";
import {
    Table,
    TableBody,
    TableCell,
...
interface ActivityLog {
    ...
    log: string;
    CompanyID: number;
    inventoryID: number; // Might be null or ID
    userID: number;
    productId: number;
    data: string; // Date
    user_name: string;
...
export default function LaptopHistory() {
    ...
    const fetchHistory = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/inventory/history/laptop/${id}`);
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            const response = await api.post(
                `/inventory/history/laptop/${id}/pdf`,
                {
                    ...
                    brand: laptop.brand,
                    model: laptop.model,
                    processor: `${laptop.processor_brand} ${laptop.processor_model}`,
                    serviceTag: laptop.service_id,
                },
                },
                { responseType: "blob" }
            );
            ...
            link.href = url;
            link.setAttribute("download", `Laptop_History_${new Date().toISOString().split("T")[0]}.pdf`);
    ...
    if (!laptop) {
        return (
            <div className="container mx-auto py-10 text-center">
                <h1 className="text-2xl font-bold text-red-500">Laptop Not Found</h1>
    ...
    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold tracking-tight">Laptop Details & History</h1>
            </div>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* <div>
                            ...
                            <p className="text-lg font-semibold">{laptop.id}</p>
                        </div> */}
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Brand & Model</p>
                            <p className="text-lg font-semibold">{laptop.brand} {laptop.model}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Processor</p>
                            <p className="text-lg font-semibold">{laptop.processor_brand} {laptop.processor_model}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Service Tag / ID</p>
                            <p className="text-lg font-semibold">{laptop.service_id || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Current Status</p>
                            <div className="mt-1">
                                {(laptop.isAvailable === 1 || laptop.isAvailable === true) ? (
                                    <span className="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold bg-green-100 text-green-800 border border-green-200">Available</span>
                                ) : (
