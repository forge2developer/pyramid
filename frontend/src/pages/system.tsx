...
export default function SystemInventory() {
    ...
    const [pyramidsIdOpen, setPyramidsIdOpen] = useState(false)
    ...
    const fetchSystem = async (_page: number = 1, searchData = formData) => {
        try {
            setLoading(true)
            setError(null)
            const { data } = await api.post(`/inventory/getAllSystems?page=${_page}&limit=${ITEMS_PER_PAGE}`, searchData)

            // Fetch Components for Table Display
            // We assume standard inventory endpoints work for getting these lists
            // Note: backend payloads might need specific filters, using empty to get all
    ...
    const handleReset = async () => {
        ...
        fetchSystem(1, emptyForm);
    }


    // --- Edit Logic ---

    const handleEditClick = (system: any) => {
        setEditingSystem(system)
        const systemId = system.systemID;

        // 1. RAMS
        const currentRams = ram.filter((r: any) => String(getValueIgnoreCase(r, 'SystemID') || getValueIgnoreCase(r, 'system_id')) === String(systemId));
        setAssignedRams(currentRams);
        setInitialAssignedRams(currentRams);


        // Available RAMs: Not assigned to any device
        ...
        // 2. SSDs
        const currentSSDs = ssd.filter((s: any) => String(getValueIgnoreCase(s, 'SystemID') || getValueIgnoreCase(s, 'system_id')) === String(systemId));
        ...
        // 3. NVMEs
        const currentNvmes = Nvme.filter((n: any) => String(getValueIgnoreCase(n, 'SystemID') || getValueIgnoreCase(n, 'system_id')) === String(systemId));
        ...
        // 4. M.2
        const currentM2s = m2.filter((m: any) => String(getValueIgnoreCase(m, 'SystemID') || getValueIgnoreCase(m, 'system_id')) === String(systemId));
    ...
    const handleSaveEdit = async () => {
        ...
        try {
            ...
            const payload: any = { ...editingSystem };
            payload.product = 'system'; // Identify type for generic backend handling if needed
            ...
            console.log("Updating System Payload:", payload);

            const { data } = await api.post("/inventory/updateSystem", payload)
            if (data.success) {
                toast.success("System updated successfully");
                setEditingSystem(null)
                fetchSystem(pagination.currentPage)
            } else {
                toast.error(data.message || "Failed to update System")
            }
        } catch (err: any) {
            console.error("Failed to update System:", err)
            toast.error(err.response?.data?.error?.message || err.response?.data?.message || err.message || "Error updating System")
        } finally {
            setIsSaving(false)
        }
    }
    ...
    const handleScrapClick = (system: any) => {
        setScrapItem(system)
    }
    ...
    const uniquePyramidId = [...new Set(monitorFilters?.map((monitor: any) => monitor.pyramidsID).filter(Boolean))]
    ...
    return (
        <div className="flex flex-col gap-3">
            <div className="rounded-xl border bg-card p-6 shadow-sm">
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
                                    ...
                                    <Command>
                                        ...
                                        <CommandList>
                                            ...
                                            <CommandGroup>
                                                ...
                                                {uniqueGeneration.map((Generation: any) => (
                                                    <CommandItem key={Generation} value={Generation} onSelect={() => { handleSelectChange("Generation", Generation); setGenerationOpen(false) }}>
                                                        <Check className={cn("mr-2 h-4 w-4", formData.Generation === Generation ? "opacity-100" : "opacity-0")} />
                                                        {Generation}
                                                    </CommandItem>
                                                ))}
                                            </CommandGroup>
                                    ...
                                    <Command>
                                        ...
                                        <CommandList>
                                            ...
                                            <CommandGroup>
                                                ...
                                                {uniqueServiceID.map((serviceID: any) => (
                                                    <CommandItem key={serviceID} value={serviceID} onSelect={() => { handleSelectChange("serviceID", serviceID); setServiceIDOpen(false) }}>
                                                        <Check className={cn("mr-2 h-4 w-4", formData.serviceID === serviceID ? "opacity-100" : "opacity-0")} />
                                                        {serviceID}
                                                    </CommandItem>
                                                ))}
                                            </CommandGroup>
                        ...
                        {/* Pyramid ID Filter */}
                        <div className="space-y-2">
                            <Label htmlFor="pyramidsID">Pyramid Id</Label>
                            <Popover open={pyramidsIdOpen} onOpenChange={setPyramidsIdOpen}>
                                <PopoverTrigger asChild>
                                    <Button variant="outline" role="combobox" aria-expanded={pyramidsIdOpen} className="w-full justify-between font-normal">
                                        {formData.pyramidsID || "Select Pyramid Id..."}
                                        <ChevronsUpDown className="ml-2 h-4 w-4 shrink-0 opacity-50" />
                                    </Button>
                                </PopoverTrigger>
                                <PopoverContent className="w-[300px] p-0">
                                    <Command>
                                        ...
                                        <CommandList>
                                            ...
                                            <CommandGroup>
                                                <CommandItem value="none" onSelect={() => { handleSelectChange("pyramidsID", ""); setPyramidsIdOpen(false) }}>
                                                    <Check className={cn("mr-2 h-4 w-4", formData.pyramidsID === "" ? "opacity-100" : "opacity-0")} />
                                                    None
                                                </CommandItem>
                                                {uniquePyramidId.map((pyramidsID: any) => (
                                                    <CommandItem key={pyramidsID} value={pyramidsID} onSelect={() => { handleSelectChange("pyramidsID", pyramidsID); setPyramidsIdOpen(false) }}>
                            ...
                            <div className="flex items-center space-x-2 h-10">
                                <Switch
                                    ...
                                    onCheckedChange={(checked) => {
                                        ...
                                        api.post("/inventory/getAllSystems", newItem).then(({ data }) => {
                                            setSystems(data.data.system);
                                            setMonitorFilters(data.data.data);
                                            setPagination(data.data.pagination);
                            ...
                            <Table className="min-w-[1000px]">
                                ...
                                <TableBody>
                                    {systems.length === 0 ? (
                                        ...
                                        systems.map((system: any, index) => (
                                            <TableRow key={system.systemID}>
                                                <TableCell className="font-medium">{(pagination.currentPage - 1) * ITEMS_PER_PAGE + index + 1}</TableCell>
                                                <TableCell>{system.pyramidsID ? system.pyramidsID : "-"}</TableCell>

                                                <TableCell
                                                    className="cursor-pointer hover:underline text-blue-600"
                                                    onClick={() => navigate(`/Inventory/system/${system.systemID}`)}
                                                >
                                                    {system.Name}
                                                    <div className={cn(
                                                        ...
                                                        system.available === "1" || system.available === "true" || system.available === "yes" || system.isAvailable
                                                            ? "bg-green-100 text-green-800 hover:bg-green-100/80"
                                                            : "bg-red-100 text-red-800 hover:bg-red-100/80"
                                                    )}>
                                                        {system.available === "1" || system.available === "true" || system.available === "yes" || system.isAvailable ? "Available" : "Moving"}
                                                    </div>
                                                </TableCell>
                                                <TableCell>{system.Processor}</TableCell>
                                                <TableCell>{system.generation || "-"}</TableCell>
                                                <TableCell>{system.serviceID}</TableCell>

                                                {/* Components Columns: matching by systemID */}
                                                <TableCell>
                                                    {ram?.filter((r: any) => String(getValueIgnoreCase(r, 'SystemID') || getValueIgnoreCase(r, 'system_id')) === String(system.systemID))
                                                        .map((r: any) => r.size).join(' + ') || '-'}
                                                </TableCell>
                                                <TableCell>
                                                    {ssd?.filter((s: any) => String(getValueIgnoreCase(s, 'SystemID') || getValueIgnoreCase(s, 'system_id')) === String(system.systemID))
                                                        .map((s: any) => s.ssdSize || s.size).join(' + ') || '-'}
                                                </TableCell>
                                                <TableCell>
                                                    {Nvme?.filter((n: any) => String(getValueIgnoreCase(n, 'SystemID') || getValueIgnoreCase(n, 'system_id')) === String(system.systemID))
                                                        .map((n: any) => n.Size || n.size).join(' + ') || '-'}
                                                </TableCell>
                                                <TableCell>
                                                    {m2?.filter((m: any) => String(getValueIgnoreCase(m, 'SystemID') || getValueIgnoreCase(m, 'system_id')) === String(system.systemID))
                                                        .map((m: any) => m.size || m.Size).join(' + ') || '-'}
                                                </TableCell>

                                                <TableCell>{system.inventoryID ? system.inventoryID : "-"}</TableCell>
                                                <TableCell>
                                                    ...
                                                    <ActionTooltip label="Scrap">
                                                        <Button variant="ghost" size="icon" onClick={() => handleScrapClick(system)}>
            ...
            <ScrapDialog
                ...
                itemName={scrapItem ? `${scrapItem.Name} ${scrapItem.serviceID ? `(${scrapItem.serviceID})` : ''}` : ''}
            />
                            ...
                            <>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            id="edit-processor"
                                            value={editingSystem.Processor}
                                            onChange={(e) => setEditingSystem({ ...editingSystem, Processor: e.target.value })}
                                        />
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        <Label htmlFor="edit-configuration">Generation</Label>
                                        <Input
                                            id="edit-configuration"
                                            value={editingSystem.generation}
                                            onChange={(e) => setEditingSystem({ ...editingSystem, generation: e.target.value })}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            id="edit-service-id"
                                            value={editingSystem.serviceID}
                                            onChange={(e) => setEditingSystem({ ...editingSystem, serviceID: e.target.value })}
                                        />
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            id="edit-pyramid-id"
                                            value={editingSystem.pyramidsID}
                                            onChange={(e) => setEditingSystem({ ...editingSystem, pyramidsID: e.target.value })}
                                        />
                                    </div>
                                    <div className="space-y-2">
                                        ...
                                        <Input
                                            ...
                                            value={editingSystem.inventoryID || ''}
                                            onChange={(e) => setEditingSystem({ ...editingSystem, inventoryID: e.target.value })}
