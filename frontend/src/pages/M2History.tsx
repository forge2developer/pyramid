...
interface M2Details {
    ...
    mw_brand: string | null;
    mw_model: string | null;
}
...
export default function M2History() {
    ...
    const [m2, setM2] = useState<M2Details | null>(null);
    ...
    const fetchHistory = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/inventory/history/m2/${id}`);
            if (response.data.success) {
                setM2(response.data.data.m_2);
    ...
    const getAssignmentInfo = (item: M2Details) => {
        ...
        if (item.mw_brand) return `Mobile Workstation: ${item.mw_brand} ${item.mw_model || ''}`;
        return "Unassigned";
    };
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            const response = await api.post(
                `/inventory/history/m2/${id}/pdf`,
                {
                    ...
                    details: {
                        ...
                        size: m2.size,
                        type: m2.type,
                        formFactor: m2.form_factor,
                        serviceTag: m2.service_id,
                    },
                },
                { responseType: "blob" }
            );
            ...
            link.href = url;
            link.setAttribute("download", `M2_History_${new Date().toISOString().split("T")[0]}.pdf`);
    ...
    if (!m2) {
        return (
            <div className="container mx-auto py-10 text-center">
                <h1 className="text-2xl font-bold text-red-500">M.2 Not Found</h1>
    ...
    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold tracking-tight">M.2 Details & History</h1>
            </div>
            ...
            <Card>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* <div>
                            <p className="text-sm font-medium text-muted-foreground">M.2 ID</p>
                            <p className="text-lg font-semibold">{m2.id}</p>
                        </div> */}
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Pyramid ID</p>
                            <p className="text-lg font-semibold">{m2.phyramidID || "-"}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Brand</p>
                            <p className="text-lg font-semibold">{m2.brand}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Size / Type / Form Factor</p>
                            <p className="text-lg font-semibold">{m2.size} - {m2.type} ({m2.form_factor})</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Service Tag</p>
                            <p className="text-lg font-semibold">{m2.service_id || "N/A"}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Inventory ID</p>
                            <p className="text-lg font-semibold">{m2.inventoryID || "-"}</p>
                        </div>
                        ...
                        <div className="col-span-1 md:col-span-2">
                            ...
                            <div className="mt-1 flex items-center gap-2">
                                ...
                                <span className="font-semibold text-primary">{getAssignmentInfo(m2)}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-1">
                                {(m2.isAvailable === 1 || m2.isAvailable === true) ? (
