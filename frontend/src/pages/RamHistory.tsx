...
import { ArrowLeft, MemoryStick, History, User, Monitor, FileText, Loader2 } from "lucide-react";
import {
    Table,
    TableBody,
    TableCell,
...
export default function RamHistory() {
    const { id } = useParams();
    const navigate = useNavigate();
    const [loading, setLoading] = useState(true);
    const [ram, setRam] = useState<RamDetails | null>(null);
    ...
    const fetchHistory = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/inventory/history/ram/${id}`);
    ...
    const getAssignmentInfo = (ram: RamDetails) => {
        if (ram.laptopID) return `Laptop: ${ram.laptop_brand} ${ram.laptop_model}`;
        if (ram.systemID) return `System: ${ram.system_name}`;
        if (ram.workstationID) return `Workstation: ${ram.workstation_processor}`;
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            link.href = url;
            link.setAttribute("download", `RAM_History_${new Date().toISOString().split("T")[0]}.pdf`);
    ...
    if (!ram) {
        return (
            <div className="container mx-auto py-10 text-center">
                <h1 className="text-2xl font-bold text-red-500">RAM Not Found</h1>
    ...
    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold tracking-tight">RAM Details & History</h1>
            </div>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* <div>
                            ...
                            <p className="text-lg font-semibold">{ram.id}</p>
                        </div> */}
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Brand</p>
                            <p className="text-lg font-semibold">{ram.brand}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Specs</p>
                            <p className="text-lg font-semibold">{ram.size} - {ram.type} - {ram.form_factor}</p>
                        </div>
                        ...
                        <div className="col-span-1 md:col-span-2">
                            ...
                            <div className="mt-1 flex items-center gap-2">
                                ...
                                <span className="font-semibold text-primary">{getAssignmentInfo(ram)}</span>
                            </div>
                        </div>
                    </div>
                </CardContent>
