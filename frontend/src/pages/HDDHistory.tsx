...
interface HDDDetails {
    ...
    warrantyEndData: string; // Note: using warrantyEndData to match DB likely
...
export default function HDDHistory() {
    const { id } = useParams();
    const navigate = useNavigate();
    const [loading, setLoading] = useState(true);
    const [hdd, setHDD] = useState<HDDDetails | null>(null);
    ...
    const fetchHistory = async () => {
        try {
            setLoading(true);
            const response = await api.get(`/inventory/history/hdd/${id}`);
            if (response.data.success) {
                setHDD(response.data.data.hdd);
    ...
    const getAssignmentInfo = (item: HDDDetails) => {
    ...
    const handleDownloadPDF = async () => {
        ...
        try {
            ...
            link.href = url;
            link.setAttribute("download", `HDD_History_${new Date().toISOString().split("T")[0]}.pdf`);
    ...
    if (!hdd) {
        return (
            <div className="container mx-auto py-10 text-center">
                <h1 className="text-2xl font-bold text-red-500">HDD Not Found</h1>
    ...
    return (
        <div className="container mx-auto py-6 space-y-6">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold tracking-tight">HDD Details & History</h1>
            </div>
                ...
                <CardContent>
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {/* <div>
                            ...
                            <p className="text-lg font-semibold">{hdd.ID}</p>
                        </div> */}
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Brand</p>
                            <p className="text-lg font-semibold">{hdd.brand}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Size / Speed</p>
                            <p className="text-lg font-semibold">{hdd.size} - {hdd.speed}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Serial Number</p>
                            <p className="text-lg font-semibold">{hdd.serialNumber}</p>
                        </div>
                        <div>
                            <p className="text-sm font-medium text-muted-foreground">Service Tag</p>
                            <p className="text-lg font-semibold">{hdd.serviceTag || "N/A"}</p>
                        </div>
                        ...
                        <div className="col-span-1 md:col-span-2">
                            ...
                            <div className="mt-1 flex items-center gap-2">
                                ...
                                <span className="font-semibold text-primary">{getAssignmentInfo(hdd)}</span>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <div className="mt-1">
                                {(hdd.isAvailable === 1 || hdd.isAvailable === true) ? (
